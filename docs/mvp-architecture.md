...
6. Mobile plays narration from `audioUrl` or uses `ttsScript` as fallback, depending on implementation.
...
- `updatedAt` is the synchronization anchor for cache refresh decisions.
- `isActive = false` means the POI should not be shown or triggered in the mobile experience.
- `audioUrl` may be empty only if the team explicitly uses `ttsScript` for narration fallback.
